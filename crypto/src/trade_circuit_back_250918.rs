use crate::{ConstraintF, utils::mimc7::*};
...
impl<F: PrimeField> TradeCircuit<F> {
    ...
    pub fn new(attr: Vec<F>, sk_s: F, nf: F, len: usize) -> Self
    where
        <F as FromStr>::Err: Debug,
    {
        Self {
        ...
        <F as FromStr>::Err: Debug,
    {
        ...
        Self {
            ...
            sk_s: Some(F::zero()),
            nf: Some(MiMC7::<F>::mimc7(F::zero(), F::zero(), &round_keys)),
            len,
            round_keys,
        }
    }
...
impl<F: PrimeField> ConstraintSynthesizer<F> for TradeCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        let nf = FpVar::<F>::new_input(cs.clone(), || {
        ...
        let mut computed_nf = mimc7_round(attr[0].clone(), sk_s.clone(), &round_keys[0]);

        for i in 1..MIMC7_ROUNDS {
            computed_nf = mimc7_round(computed_nf, sk_s.clone(), &round_keys[i]);
        }
        ...
        /**
         * 1. commitment (attr) 확인 -> CPSNARK
         * 2. nullifier (cm, sk_s) 확인 -> OK
         * 3. encryption (attr, o) 확인 -> TANGRAM?
         */
        Ok(())
    }
}
...
mod trade_circuit {
        ...
        <E::ScalarField as FromStr>::Err: Debug,
    {
        ...
        // setup
        let circuit = TradeCircuit::<E::ScalarField>::mock(LEN);
        ...
        let circuit = TradeCircuit::<E::ScalarField>::new(attr.clone(), sk_s, nf, LEN);
