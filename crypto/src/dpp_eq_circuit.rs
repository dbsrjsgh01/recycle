...
#[derive(Clone, Debug)]
pub struct DPPEqCircuit<F: PrimeField> {
...
impl<F: PrimeField> DPPEqCircuit<F> {
...
impl<F: PrimeField> ConstraintSynthesizer<F> for DPPEqCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        for i in 0..self.len {
            let mut res = chk1[i]
                .clone()
                .not()
                .or(&chk1[i].and(&attr[i].is_eq(&cond[i]).unwrap()).unwrap())
                .unwrap();
...
mod dpp_circuit {
    ...
    fn test_dpp_circuit() {
        ...
        let circuit = DPPEqCircuit::<F>::new(attr.clone(), cond.clone(), chk1, LEN);
    ...
    fn test_cp_dpp<E: Pairing>() {
        ...
        let cond = vec![E::ScalarField::from(2u64); LEN]; // [1, 1, ..., 1]

        let mut chk1 = vec![true; LEN / 2]; // [1, 1, ..., 1]
        chk1.extend_from_slice(&[false; LEN / 2]); // [0, 0, ..., 0]

        // setup
        let circuit = DPPEqCircuit::<E::ScalarField>::mock(LEN, cond.clone());
        ...
        let circuit = DPPEqCircuit::<E::ScalarField>::new(attr.clone(), cond.clone(), chk1, LEN);
